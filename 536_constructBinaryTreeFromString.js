...
var str2tree = function (s) {
    if (s.length === 0) return null;//boundary case 
    let _CC_A = ["0", "9", "(", ")", "-"].map(it_s => it_s.charCodeAt(0));
    let _stack_a = [new TreeNode(Infinity)];
    for (let c_s of [...s]) {
        let c_n = c_s.charCodeAt(0);
        let currNode = _stack_a[_stack_a.length - 1];
        if (c_n >= _CC_A[0] && c_n <= _CC_A[1]) {
            if (currNode.val === Infinity) currNode.val = parseInt(c_s);
            else if (currNode.val === -Infinity) currNode.val = -parseInt(c_s);
            else if (currNode.val > 0) currNode.val = currNode.val * 10 + parseInt(c_s);//append digit
            else currNode.val = currNode.val * 10 - parseInt(c_s);//append digit
        }
        else if (c_n === _CC_A[4])
            currNode.val *= -1;//negative sign
        else if (c_n === _CC_A[2]) {//enters new stack
            let _newNode = new TreeNode(Infinity);
            if (currNode.left === null)
                currNode.left = _newNode;
            else
                currNode.right = _newNode;
            _stack_a.push(_newNode);
            currNumber_a = ["", false];//clears 
        } else if (c_n === _CC_A[3]) {//exiting current stack
            if (_stack_a.length > 1) _stack_a.pop();
        } else throw "error";
    }
    let i = 0;
    return _stack_a[0];
