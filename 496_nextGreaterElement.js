 ...
 * @return {number[]}
 */
var nextGreaterElement = function (nums1_nA, nums2_nA) {
    /**
     * nums1 is the array to be hashed
     * nums2 is the array to be iterated
     * 
     * 错误1：
     * Monotonic Stack在发现当前元素小于ms最后一个元素时，不要全部pop出来
     * 因为虽然最后一个元素比当前元素小，不能保证所有元素比当前元素小
     */
    let ret_nA = Array(nums1_nA.length).fill(-1);
    let nums1EleToInd_M = new Map();
    nums1_nA.forEach((it_n, ind_n) => {
        //nums1 element guarenteed to be distinct
        nums1EleToInd_M.set(it_n, ind_n);
    });
    let monoStack_nA = [];
    nums2_nA.forEach((it_n) => {
        while (monoStack_nA.length > 0 && it_n > monoStack_nA[monoStack_nA.length - 1]) {//!
            let currMonoStackElem_n = monoStack_nA.pop();
            ret_nA[nums1EleToInd_M.get(currMonoStackElem_n)] = it_n;
        }
        if (nums1EleToInd_M.has(it_n)) {
            monoStack_nA.push(it_n);
        }
    });
    return ret_nA;
};
